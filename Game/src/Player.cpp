...
void Player::Move(Direction direction, float dt)
{
	...
	if (_transform.position.x > CAMERA_SIZE) _transform.position.x = CAMERA_SIZE;
	if (_transform.position.y > CAMERA_SIZE) _transform.position.y = CAMERA_SIZE;
	if (_transform.position.x < CAMERA_SIZE * -1) _transform.position.x = CAMERA_SIZE * -1;
	if (_transform.position.y < CAMERA_SIZE * -1) _transform.position.y = CAMERA_SIZE * -1;
}
...
bool Player::CheckCollision(Vector3 pos)
{
	SDL_Rect* result = new SDL_Rect();

	Vector3 objPos = _projectile->GetTransform().position;
